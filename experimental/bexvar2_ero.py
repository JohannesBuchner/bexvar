...
def bexvar(log_src_crs_grid, pdfs):
    ...
    numerical_pdfs = []
    for pdf_in in pdfs:
        pdf = pdf_in / pdf_in.sum()
        gauss_mean = np.average(log_src_crs_grid, weights=pdf)
        gauss_sigma = np.sqrt(np.cov(log_src_crs_grid, aweights=pdf))
        rv = scipy.stats.norm(gauss_mean, gauss_sigma)
        gauss_logpdf = rv.logpdf(log_src_crs_grid)
        gauss_pdf = rv.pdf(log_src_crs_grid)
        gauss_norm = gauss_pdf.sum()
        surprise = np.sum(((gauss_logpdf - np.log(gauss_norm)) / np.log(2) - np.log2((pdf + 1e-300))) * (gauss_pdf / gauss_norm))
        print("checking whether analytic integration is possible...", surprise, gauss_pdf[0] / (gauss_pdf.max()))
        if surprise < 0.1 and gauss_pdf[0] < gauss_pdf.max() / 100:
            gaussian_means.append(gauss_mean)
            gaussian_stdevs.append(gauss_sigma)
        else:
            numerical_pdfs.append(pdf_in)

    numerical_pdfs = np.array(numerical_pdfs)
    gaussian_means = np.array(gaussian_means)
    gaussian_stdevs = np.array(gaussian_stdevs)
    has_numerical_pdfs = len(numerical_pdfs) > 0
    has_gaussian_pdfs = len(gaussian_stdevs) > 0
    print("using %d analytic, %d numerical integrals" % (len(gaussian_stdevs), len(numerical_pdfs)))
    ...
    def loglike(params):
        ...
        if has_numerical_pdfs:
            ...
            numerical_likes = log((variance_pdf.reshape((1, -1)) * numerical_pdfs).mean(axis=1) + 1e-100)
            like += numerical_likes.sum()

        if has_gaussian_pdfs:
            # compute integral of the product of two gaussians analytically
            total_variance = log_sigma**2 + gaussian_stdevs**2
            gauss_likes = -0.5 * (log_mean - gaussian_means)**2 / total_variance - 0.5 * np.log(2 * np.pi * total_variance)
            like += gauss_likes.sum()
